...
The actual request/SSE-parsing mechanics live in
``adapters/openai_compatible/completion.py`` — shared with
``GroqCompletionAdapter``, which speaks the same wire format against a
different base URL. This class is now a thin, provider-specific wrapper
around that shared function (base URL + capability registry only);
behavior (request shape, error format, retryability) is unchanged.
"""
