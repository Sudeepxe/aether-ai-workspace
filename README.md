...
> **Status: Sprint 7 — Eval harness + real baseline complete.** Sprints 0–6
> (factory; identity & forced row-level-security tenant isolation;
...
> ingestion pipeline; grounded chat — hybrid retrieval, RRF/MMR, dual-feed
> query rewrite, two-gate refusal, per-chunk citations, citations/refusal UI)
> are merged to `main`. Sprint 7 builds the real evaluation harness (§6.4):
> a runner that drives the actual deployed HTTP surface end to end (real
> ingestion, real retrieval, real chat) against a 20-case golden set across
> four classes (answerable/unanswerable/adversarial/multi-turn); mechanical
> metrics needing no LLM (refusal correctness, retrieval hit-rate, citation
> precision/recall — all **live at 100%** against real output); an
> LLM-judge faithfulness scorer with real, code-enforced cross-family
> judging (ADR-6.5); a real, data-derived retrieval-threshold calibration
> replacing Sprint 6's placeholder; CI wiring (path-filtered PR smoke,
> scheduled nightly, both gating on real regressions); and a
> [published report](docs/evals/latest-report.md) generated by an actual
> run, not hand-written. The one number Sprint 7 does **not** claim: the
> North Star (§1.7, faithfulness ≥ 90% ∧ correct-refusal ≥ 90%) is honestly
> reported as **not yet determinable** — it needs a real cross-family LLM
> judge, and no OpenAI/Anthropic key is provisioned in this environment.
> Every other AI-facing component (chat generator, embedding call, query
> rewrite) likewise falls back to honest, clearly-labeled local placeholders
> (grounding-aware echo completion, deterministic non-semantic embeddings, a
> no-op rewriter) in dev/CI — real providers activate automatically the
> moment the corresponding API key is set, no code change required, and the
> harness will compute a genuine North Star score the moment one is. This
